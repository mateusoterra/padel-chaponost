...
export default function Home() {
  ...
  const addGuest = async (game) => {
    const name = prompt("Nom de l'invité");
    if (!name) return;

    await updateDoc(doc(db, "games", game.id), {
      guests: [...(game.guests || []), name]
    });
  };
  ...
  const generateMatch = async (game) => {
    if (game.participants.length === 0) {
      alert("Au moins 1 joueur inscrit est requis");
      return;
    }
    let interessados = players.filter(p => game.participants.includes(p.uid));
    interessados.sort((a, b) => score(a) - score(b));

    let selecionados = interessados.slice(0, 4);

    // evita duplicação
    selecionados = [...new Map(selecionados.map(p => [p.uid, p])).values()];
  ...
  const confirmGame = async (game) => {
    ...
    for (let uid of game.selected) {
      ...
      await updateDoc(doc(db, "players", player.id), {
        presencas: (player.presencas || 0) + 1
      });
    }
...
const header = (
  <div style={{ marginBottom: 10 }}>
    <div style={{ width: "100%", height: 180, overflow: "hidden" }}>
      <img src="/logo.png" style={{ width: "100%", height: "120%", objectFit: "cover" }} />
    </div>
  </div>
);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.content}>
        ...
        <button style={styles.button} onClick={() => setView("history")}>Historique</button>
        ...
        {games.filter(g => g.status === "open" || g.status === "ready").map(game => {
          ...
          return (
            <div key={game.id} style={styles.card}>
              ...
              {(game.participants.length + (game.guests?.length || 0)) >= 4 && game.status === "open" && (
